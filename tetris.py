...
class Tetris:
    ...
    def __init__(self, height=20, width=10, block_size=20):
        ...
        self.extra_board = np.ones((self.height * self.block_size, self.width * int(self.block_size / 1.5), 3),
                                   dtype=np.uint8) * np.array([125, 125, 125], dtype=np.uint8)
    ...
    def reset(self):
        ...
        self.next_ind = random.randint(0, len(self.pieces)-1)
    ...
    def get_state_properties(self, board, next=False):
        lines_cleared, board = self.check_cleared_rows(board)
        holes = self.get_holes(board)
        bumpiness, height = self.get_bumpiness_and_height(board)
        piece_index = self.next_ind if next else self.ind
        #state = torch.FloatTensor([piece_index+1, lines_cleared, *holes, *bumpiness, *height])
        state = torch.FloatTensor([piece_index+1, lines_cleared, holes, bumpiness, height])
        return state
    ...
    def _get_holes(self, board):
        ...
        for i, col in enumerate(cols):
            ...
            while row < self.height and col[row] == 0:
                row += 1
            holes.append(len([x for x in col[row + 1:] if x == 0]))
    ...
    def _get_bumpiness_and_height(self, board):
        board = np.array(board)
        mask = board != 0
        invert_heights = np.where(mask.any(axis=0), np.argmax(mask, axis=0), self.height)
        heights = self.height - invert_heights
    ...
    def get_next_states(self):
        ...
        for i in range(num_rotations):
            ...
            for x in range(valid_xs + 1):
                ...
                states[(x, i)] = self.get_state_properties(board, next=True)
    ...
    def step(self, action, render=True, video=None):
        ...
        score = 1 + (lines_cleared) * self.width
        self.score += score
        self.tetrominoes += 1
    ...
    def render(self, video=None):
        ...
        scorefont = cv2.FONT_HERSHEY_SCRIPT_SIMPLEX

        cv2.putText(img, "Score", (self.width * self.block_size + int(self.block_size / 2), 2*self.block_size),
                    fontFace=font, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.score),
                    (self.width * self.block_size + int(self.block_size / 2), 4 * self.block_size),
                    fontFace=scorefont, fontScale=1.0, color=self.text_color)

        cv2.putText(img, "Pieces", (self.width * self.block_size + int(self.block_size / 2), 7 * self.block_size),
                    fontFace=font, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.tetrominoes),
                    (self.width * self.block_size + int(self.block_size / 2), 9 * self.block_size),
                    fontFace=scorefont, fontScale=1.0, color=self.text_color)

        cv2.putText(img, "Lines", (self.width * self.block_size + int(self.block_size / 2), 12 * self.block_size),
                    fontFace=font, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.cleared_lines),
                    (self.width * self.block_size + int(self.block_size / 2), 14 * self.block_size),
                    fontFace=scorefont, fontScale=1.0, color=self.text_color)
...
if __name__ == '__main__':
    ...
    for i in range(len(game.pieces)):
        print("{} -> {}".format(game.pieces[i], game.rotate(game.pieces[i])))
